...
<script>
import { biProductSatisfactionTableAPI } from '@/api/bi/bi.js'
            ...
            for (const key in firstItem) {
              if (key !== 'productName') {
                this.detailFields.push({
                  name: key,
                  crmType: 'contract',
                  fieldType: 'productPopular',
                  // difType: 'product',
                  request: biProductSatisfactionTableAPI,
                  params: {
                    searchType: key === 'visitNum' ? null : key
                  }
                })
              }
              if (reg.test(key)) {
    ...
    handleRowClick(row, column) {
      if (this.getDetailFieldList(column)) {
        const params = {
          ...
          page: 1,
          search: '',
          searchType: column.property == 'visitNum' ? null : column.label,
          type: 6
        }
