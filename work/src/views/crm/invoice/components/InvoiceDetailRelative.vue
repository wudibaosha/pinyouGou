...
<script>
import { crmInvoiceQueryOtherContractAPI } from '@/api/crm/invoice'

export default {
  name: 'InvoiceDetailRelative',
  components: {
    CRMAllDetail: () =>
  ...
  methods: {
    ...
    getContractOther() {
      const params = {
        pageType: 0,
        invoiceId: this.detail.invoiceId
      }
      crmInvoiceQueryOtherContractAPI(params).then(res => {
