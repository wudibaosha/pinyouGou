  ...
  computed: {
    ...
    tabNames() {
      const tempsTabs = [
        ...
        { label: '回款', name: 'RelativeReturnMoney' },
    ...
    getDetial() {
      ...
      crmProjectReadAPI(this.id)
        .then(res => {
          ...
          this.headDetails = [
            { title: '项目批次号', value: resData.sampleBatchNum },
            { title: resData.type == '启动' ? '启项目动类型' : '释放内容', value: resData.type == '启动' ? resData.startProType : resData.releaseContent },
            { title: '申请'+resData.type == '启动'?'启动':'交付'+'日期', value: resData.createTime },
            { title: resData.type == '启动'?'启动金额':'交付金额', value: resData.type == '启动'?resData.startMoney:releaseMoney },
