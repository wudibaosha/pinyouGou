...
import { crmSettlementFileListAPI } from '@/api/crm/settlement'
  ...
  methods: {
    getDetail(loading = true) {
      ...
      const request = {
        ...
        settlement: crmSettlementFileListAPI
      }[this.crmType]
      const params = {}
      params['id'] = this.id
      request(params)
    ...
    uploadFile(event) {
      ...
      for (let index = 0; index < files.length; index++) {
        ...
        if(crmType[this.crmType] !== undefined){
          param.label = crmType[this.crmType]
        }
