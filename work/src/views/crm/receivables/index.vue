<template>
  <div>
    <wk-page-header
      :title="config.showModuleName ? '回款管理' : ''"
      ...
      <template slot="right">
        <el-button
          v-if="saveAuth"
          type="primary"
          @click="createClick">新建回款</el-button>
      </template>
    </wk-page-header>
    <div
      v-empty="!indexAuth"
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="{ row, column, $index }">
            ...
            <template v-else-if="item.prop == 'invoiceName'">
              <span :style="getStatusStyle2(row.checkStatus)">{{ row.invoiceName }}</span>
            </template>
            <template v-else-if="item.prop == 'invoiceStatus'">
              <span :style="getStatusStyle(row.invoiceStatus)" class="status-mark" />
              <span>{{ getStatusName(row.invoiceStatus) }}</span>
            </template>
            <wk-field-view
              v-else
              :props="item"
          ...
          <template slot-scope="scope">
            <div style="display: flex;justify-content: space-between;width: 100%;">
              <el-button
                type="primary-text"
                style="padding: 0;"
                @click.native="markReceivables(scope)">{{ scope.row.timeUpdated == 1 ? '已更新回款时间': '更新回款时间' }} </el-button>
              </div>
            </template>
        </el-table-column>
      ...
      <div class="p-contianer">
        ...
        <span v-if="moneyPageData" class="money-bar">回款金额<i class="wk wk-icon-fill-help wk-help-tips" data-type="12" data-id="151" />：{{ moneyPageData.receivablesMoney || 0 | separator }}</span>
      </div>
    </div>

    <update-return-time
      v-if="updateReturnTimeShow"
      :visible.sync="updateReturnTimeShow"
      :detail="rowDetail"
      @change="handleHandle2()"
    />
...
import UpdateReturnTime from './components/UpdateReturnTime.vue'
...
import { floatAdd, objDeepCopy } from '@/utils'
...
export default {
    ...
    moneyPageData() {
      ...
      if (this.selectionList.length == 0) {
        ...
        for (let index = 0; index < this.selectionList.length; index++) {
          ...
          // 1 审核通过的 10 正常
          if (element.checkStatus == 1 || element.checkStatus == 10) {
  ...
  methods: {
    ...
    markReceivables(scope){
      this.rowDetail = objDeepCopy(scope.row)
      console.log(this.rowDetail)
      this.updateReturnTimeShow = true
    },
    /**
     * @description: 表头事件
     * @param {*} type
    ...
    tableOperationsClick(type) {
      if (type === 'transfer') {
        ...
        // 场景编辑请求
          request: crmReceivablesTransferAPI,
        ...
        this.$wkExport.export(this.crmType, {
          params: { ids: this.selectionList.map(item => item[`${this.crmType}Id`]) },
          request: crmReceivablesExcelExportAPI
        })
      } else if (type === 'delete') {
        this.$confirm(`确定删除选中的${this.selectionList.length}项吗？`, '提示', {
          ...
          .then(() => {
            this.loading = true
            crmReceivablesDeleteAPI(this.selectionList.map(item => item[`${this.crmType}Id`]))
              .then(res => {
                this.loading = false
                this.$message({
                  type: 'success',
        ...
        this.teamsHandleProps = {
          ...
          addRequest: crmReceivablesSettingTeamSaveAPI,
          removeRequest: crmReceivablesSettingTeamDeleteAPI,
          params: { ids: this.selectionList.map(item => item[this.crmType + 'Id']) },
          // 帮助提示
