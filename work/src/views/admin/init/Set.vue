<template>
  <div v-loading="loading" class="init-set main">
    <xr-header>
      <template slot="label">初始化数据<span class="label-des">（试用悟空CRM后需要删除测试数据，正式管理企业信息）</span></template>
    </xr-header>
    <div class="main-bod">
      <reminder
        content="1、可以对单个应用或多个应用初始化，应用下的数据将全部删除；<br />2、注意初始化单个应用后，将影响其他应用下与此应用关联的数据；<br /><span style='color: red;'>初始化的数据不可恢复，请谨慎操作！</span>"
        class="xr-reminder" />
      ...
      <div class="init__body">
        ...
        <el-button :disabled="value.length == 0" class="save-btn" type="primary" @click="saveClick">确定初始化</el-button>
      </div>
    </div>
  </div>
</template>
