...
export function timeToFormatTime(time, format) {
  if (time) {
    let momentObj = ''
    if (isNaN(time)) { // 如果误传非数字时间，直接当做格式化时间处理
...
export function getBaiduMap() {
  if (!global.BMap) {
    ...
    global.BMap._preloader = new Promise((resolve, reject) => {
      ...
      $script.src = `https://api.map.baidu.com/api?v=3.0&ak=bWl9uAQdtBVLOUPhZrDF3RLkG6bvX7PU&callback=_initBaiduMap`
    })
