...
export default {
  ...
  methods: {
    ...
    getItemValue(item, detail, type) {
      ...
      if (item.formType == 'contacts' ||
      ...
      } else if (item.formType == 'product' || item.formType == 'contractRelation') {
        // 关联产品
        return item.value
        ...
        if (type === 'update') {
          ...
          return isArray(item.defaultValue) && item.defaultValue.length > 0
            ...
            item.formType == 'file') {
        if (type === 'update') {
          console.log(objDeepCopy(item.value))
          return item.value ? objDeepCopy(item.value) : []
        } else {
      ...
      if (valueObj) {
        this.itemsForEach(list, item => {
          if (!valueObj.hasOwnProperty(item.field)) {
            valueObj[item.field] = item.defaultValue
          }
        })
      }
      this.itemsForEach(list, item => {
        if (defaultValueObj) {
          defaultValueObj[item.field] = item.defaultValue
        }
        if ([
          'select',
          'checkbox'
     ...
     this.itemsForEach(fieldList, fieldItem => {
        ...
        if (fieldItem.show) {
          ...
          if (formObj.hasOwnProperty(fieldItem.field)) {
            ...
            //当fieldItem.field为money的时候，formObj[fieldItem.field]值赋值给money属性
            
            fieldForm[fieldItem.field] = formObj[fieldItem.field]
            // this.$set(fieldForm, 'fieldAinmgu', formObj['fieldAinmgu'])
            this.$nextTick(function(){
              fieldForm['fieldAinmgu'] = formObj['fieldAinmgu']  
            })
